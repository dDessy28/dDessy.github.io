...
function showHide(id) {
	...
	if (artikelExists("artikel/" + id + ".json")) {
		var jArtikel = readFile("artikel/" + id + ".json");
	...
	for (i = 0; i < 20; i++) {
		if(artikelExists(dirArtikel + i + ".json")) {
			...
			parseKonten.forEach(element => {
				var features = panelKepala + element.judul + panelMeta + "Penulis : " + element.penulis + " Tanggal : " + element.tanggal + panelBadan + element.deskripsi + "..." + panelFoot + " id='" + i + "' onclick='showHide(this.id)'" + panelClick + panelAkhir;
